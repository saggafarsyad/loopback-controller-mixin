...
function prepare(definition) {
  ...
  if (remoteMethod.acls && typeof remoteMethod.acls === 'object')
    remoteMethod.acls = [remoteMethod.acls]
  else if (!remoteMethod.acls)
    remoteMethod.acls = []
  // --- Set default acl value
  remoteMethod.acls = remoteMethod.acls.map(acl => {
