 ...
 * @param {Object} options.acl Options for ACL
 * @param {string} options.acl.principalId Default principal ID
 */
function Controller(Model, options) {
  // If Model is exposed to public, apply mixin
  if (Model && Model.sharedClass) {
    debug('Loading Controller. Model: %s', Model.modelName)
    // If options is unset or set to true, init new objet
    if (!options || options === true)
      options = {}
    // Add modelName to option
    ...
    controllers = keys.map(name => {
      ...
      return prepare(controller, options)
    })
    // Load controllers
    ...
    debug('Done loading Controller. Model: %s', Model.modelName)
  }
}
 ...
 * @param {Object} options.acl Options for ACLs
 * @param {string} options.acl.defaultPrincipal Default ACL principal
 */
function prepare(definition, options) {
  ...
  if (!remoteMethod.http)
    remoteMethod.http = {}
  // Get verb and path
  // If verb not defined, get from path
  let verb, path
  // If path is undefined, throw error
  if (!definition.path)
    throw new Error('Path is not defined. Endpoint: ' + definition.name)
  // If verb is undefined, get verb from path or set default
  if (!definition.verb) {
    // Split path using space
    let tmp = definition.path.split(' ', 2)
    // If no verb in path, set GET as default verb
    if (tmp.length === 1) {
      verb = 'GET'
      path = tmp[0]      
    } else {
  ...
  remoteMethod.http.verb = verb.toLowerCase()
  remoteMethod.http.path = path
  // -- Map description
  ...
  let acls = definition.acls.map(acl => {
    ...
    // Set default principalId
    if (!acl.principalId) {
      // If default is unset, set to model
      if (!options.acl || !options.acl.defaultPrincipal)
        acl.principalId = options.modelName
      // Else, set to default principal
      else
        acl.principalId = options.acl.defaultPrincipal
    }
...
function loadController(model, controllers) {
  ...
  for (let i = 0; i < len; i++) {
    ...
    let name = def.name
    debug('Controller "%s", definition: %j', name, def)    
